...
def _render_tela_cadastro() -> None:
    ...
    with st.form("form_cadastro_fornecedor"):
        ...
        email = st.text_input(
            ...
            help="Opcional. Usado só se 2FA por e-mail estiver ligado no futuro.",
        )
        senha = st.text_input(
            "Senha",
            type="password",
...
def _render_tela_redefinir() -> None:
    ...
    st.markdown(
        f"""
        <div class="login-box">
            <h2>🔄 {"Troca obrigatória de senha" if forcar else "Redefinir senha"}</h2>
...
def _render_criar_senha_link(info: dict[str, str], po: str, linha: str) -> None:
    ...
    with st.form("criar_senha_link_fornecedor"):
        ...
        email = st.text_input("E-mail corporativo (recomendado)")
        senha = st.text_input("Senha", type="password")
